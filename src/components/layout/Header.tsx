...
const Header: React.FC<HeaderProps> = ({ currentPage = "Home" }) => {
  ...
  return (
    ...
    >
      <nav
        className={`container mx-auto px-4 py-2 flex items-center justify-between border-secondary transition-all duration-300 ${
          ...
          {navItems.map((item, index) => (
            <Link
              ...
              className={`relative text-lg py-2 transition-colors text-primary group ${
                currentPage === item.name ? "font-semibold" : ""
              }`}
            >
              {item.name}
              <span
